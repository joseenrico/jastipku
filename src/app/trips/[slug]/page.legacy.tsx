...
// High-quality gallery images for each trip
...
export default function TripDetailPage() {
  ...
  const gallery = trip ? (galleryImages[trip.id] || [{ src: (trip as any).imageUrl || "", alt: trip.destination }]) : [{ src: "", alt: "" }];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      ...
      router.push(`/login?next=/trips/${encodeURIComponent(slug)}`);
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      ...
      <div className="relative h-[600px] w-full overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-30 p-6 flex items-center justify-between">
          <Button variant="ghost" size="icon" className="rounded-full bg-black/40 hover:bg-black/60 text-white backdrop-blur-sm" onClick={() => router.back()}>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className={`text-muted-foreground leading-relaxed ${!isExpanded ? "line-clamp-3" : ""}`}>
                    {(trip as any).description || `Join our exciting jastip trip to ${trip.destination}! We'll be shopping for the best products and bringing them back to you.`}
                  </p>
                  <Button
                    variant="ghost"
                    size="sm"
